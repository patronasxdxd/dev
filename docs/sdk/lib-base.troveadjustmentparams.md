...
Defining both `depositCollateral` and `withdrawCollateral`<!-- -->, or both `borrowTHUSD` and `repayTHUSD` at the same time is disallowed, and will result in a type-checking error.
...
<tr> <td> borrowTHUSD? </td> <td> T </td> <td> <i>(Optional)</i> The amount of thUSD that's borrowed. </td> </tr>

<tr> <td> repayTHUSD? </td> <td> T </td> <td> <i>(Optional)</i> The amount of thUSD that's repaid. </td> </tr>
