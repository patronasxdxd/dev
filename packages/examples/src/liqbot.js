...
const { Decimal, UserTrove, THUSD_LIQUIDATION_RESERVE } = require("@threshold-usd/lib-base");
const { EthersLiquity, EthersLiquityWithStore } = require("@threshold-usd/lib-ethers");
...
async function tryToLiquidate(liquity) {
  ...
  try {
    ...
    const expectedCompensation = total.collateral
      ...
      .add(THUSD_LIQUIDATION_RESERVE.mul(troves.length));
    ...
    const { collateralGasCompensation, thusdGasCompensation, liquidatedAddresses } = receipt.details;
    ...
    const totalCompensation = collateralGasCompensation
      .mul(store.state.price)
      .add(thusdGasCompensation);

    success(
      `Received ${bold(`${collateralGasCompensation.toString(4)} ETH`)} + ` +
        `${bold(`${thusdGasCompensation.toString(2)} THUSD`)} compensation (` +
