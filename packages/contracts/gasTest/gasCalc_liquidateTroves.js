...
contract('Gas cost tests', async accounts => {
  ...
  it("", async () => {
    ...
    // 10 accts each open Trove with 10 ether, withdraw 100 THUSD
  ...
  it("", async () => {
    ...
    //60 accts open Trove with 1 ether and withdraw 100 THUSD
  ...
  it("", async () => {
    ...
    // 10 accts each open Trove with 15 ether, withdraw 100 THUSD
    await th.openTrove_allAccounts(accounts.slice(101, 111), contracts, dec(15, 'ether'), dec(100, 18))

    //65 accts open Trove with 1 ether and withdraw 100 THUSD
  ...
  it("", async () => {
    ...
    //50 accts open Trove with 1 ether and withdraw 100 THUSD
    const _50_Defaulters = accounts.slice(1, 51)
    await th.openTrove_allAccounts(_50_Defaulters, contracts, dec(100, 'ether'), dec(9500, 18))
  ...
  it("", async () => {
    ...
    await borrowerOperations.openTrove(_100pct, dec(1, 27), accounts[999],ZERO_ADDRESS, { from: accounts[999], value: dec(1, 27) })
    await stabilityPool.provideToSP(dec(1, 27), { from: accounts[999] })
    assert.equal((await stabilityPool.getTotalTHUSDDeposits()), dec(1, 27))
  ...
  it("", async () => {
    ...
    // 10 accts each open Trove with 10 ether, withdraw 2000 THUSD
    await th.openTrove_allAccounts(accounts.slice(101, 111), contracts, dec(200, 'ether'), dec(2000, 18))

    // Account 500 opens with 1 ether and withdraws 2000 THUSD
    await borrowerOperations.openTrove(_100pct, dec(2000, 18), accounts[500],ZERO_ADDRESS, { from: accounts[500], value: dec(20, 'ether') })
    assert.isTrue(await sortedTroves.contains(accounts[500]))
