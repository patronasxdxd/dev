...
contract LiquityBase is BaseMath, ILiquityBase {
    ...
    uint constant public THUSD_GAS_COMPENSATION = 200e18;
    ...
    function _getCompositeDebt(uint _debt) internal pure returns (uint) {
        return _debt.add(THUSD_GAS_COMPENSATION);
    }

    function _getNetDebt(uint _debt) internal pure returns (uint) {
        return _debt.sub(THUSD_GAS_COMPENSATION);
    }
    ...
    function getEntireSystemDebt() public view returns (uint entireSystemDebt) {
        uint activeDebt = activePool.getTHUSDDebt();
        uint closedDebt = defaultPool.getTHUSDDebt();
