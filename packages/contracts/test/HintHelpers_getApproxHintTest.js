...
contract('HintHelpers', async accounts => {
  ...
  /* Open a Trove for each account. THUSD debt is 200 THUSD each, with collateral beginning at
  1.5 ether, and rising by 0.01 ether per Trove.  Hence, the ICR of account (i + 1) is always 1% greater than the ICR of account i.
 */

 // Open Troves in parallel, then withdraw THUSD in parallel
 const makeTrovesInParallel = async (accounts, n) => {
  ...
  const withdrawTHUSDpromises = activeAccounts.map(account => withdrawTHUSDfromTrove(account))
  await Promise.all(withdrawTHUSDpromises)
 ...
 const withdrawTHUSDfromTrove = async (account) => {
  await borrowerOperations.withdrawTHUSD(th._100pct, '100000000000000000000', account, account, { from: account })
 }

 // Sequentially add coll and withdraw THUSD, 1 account at a time
  const makeTrovesInSequence = async (accounts, n) => {
    ...
    for (const account of activeAccounts) {
      ...
      await th.openTrove(contracts, { extraTHUSDAmount: toBN(dec(10000, 18)), ICR: ICR_BN, extraParams: { from: account } })
