...
const queryDataArgs = abiCoder.encode(["string", "string"], ["erc20", "usd"]);
...
contract('PriceFeed', async accounts => {
  ...
  const contextTestPriceFeed = (tellorDigits) => {
    ...
    async function setTellorPrice(price) {
      // Tests set price with 6 digits, adjust it if tellorDigits bigger than that
      adjustedPrice = dec(price, tellorDigits - 6)  
      let valueBytes = abiCoder.encode(["uint256"], [adjustedPrice]);
      await mockTellor.submitValue(queryId, valueBytes, 0, queryData)
    ...
    beforeEach(async () => {
      ...
      mockTellor = await MockTellor.new(queryData)
      MockTellor.setAsDeployed(mockTellor)

      tellorCaller = await TellorCaller.new(mockTellor.address, queryId)
      ...
      await setTellorPrice(dec(100, 18))
    ...
    it("C1 Chainlink breaks, Tellor working: fetchPrice should return the correct Tellor price, taking into account Tellor's 6-digit granularity", async () => {
      ...
      await setTellorPrice(dec(123, 6))
      ...
      await setTellorPrice(dec(1234567890))
      await priceFeed.fetchPrice()
      price = await priceFeed.lastGoodPrice()
      // Check Liquity PriceFeed gives 0.0001 with 18 digit precision
      assert.equal(price, '1234567890000000000000')
    ...
    it("C1 chainlinkWorking: Chainlink frozen, Tellor frozen: switch to usingTellorChainlinkFrozen", async () => {
      ...
      const now = await th.getLatestBlockTimestamp(web3)
      const tellorUpdateTime = (await tellorCaller.getTellorCurrentValue.call())[2]
    ...
    it("C1 chainlinkWorking: Chainlink price drop of >50%, switch to usingChainlinkTellorUntrusted", async () => {
      ...
      await setTellorPrice(dec(203, 4))
    ...
    it("C1 chainlinkWorking: Chainlink price drop of >50% and Tellor price matches: remain chainlinkWorking", async () => {
      ...
      await mockChainlink.setPrevPrice(dec(2, 8))  // price = 2
      await mockChainlink.setPrice(99999999)  // price drops to 0.99999999: a drop of > 50% from previous
      await setTellorPrice(999999) // Tellor price drops to same value (6 ecimals)
    ...
    it("C1 chainlinkWorking: Chainlink price drop of >50% and Tellor price matches: return Chainlink price", async () => {
      ...
      await mockChainlink.setPrevPrice(dec(2, 8))  // price = 2
      await mockChainlink.setPrice(99999999)  // price drops to 0.99999999: a drop of > 50% from previous
      await setTellorPrice(999999) // Tellor price drops to same value (at 6 decimals)
    ...
    it("C1 chainlinkWorking: Chainlink price drop of >50% and Tellor price within 5% of Chainlink: remain chainlinkWorking", async () => {
      ...
      await setTellorPrice(104999999) // Tellor price drops to 104.99: price difference with new Chainlink price is now just under 5%
    ...
    it("C1 chainlinkWorking: Chainlink price drop of >50% and Tellor live but not within 5% of Chainlink: switch to usingChainlinkTellorUntrusted", async () => {
      ...
      await mockChainlink.setPrevPrice(dec(1000, 8))  // prev price = 1000
      await mockChainlink.setPrice(dec(100, 8))  // price drops to 100: a drop of > 50% from previous
      await setTellorPrice(105000001) // Tellor price drops to 105.000001: price difference with new Chainlink price is now > 5%
    ...
    it("C1 chainlinkWorking: Chainlink price drop of >50% and Tellor frozen: switch to usingChainlinkTellorUntrusted", async () => {
      ...
      await mockChainlink.setPrevPrice(dec(1000, 8))  // prev price = 1000
      await mockChainlink.setPrice(dec(100, 8))  // price drops to 100: a drop of > 50% from previous
      await setTellorPrice(dec(100, 8))
    ...
    it("C1 chainlinkWorking: Chainlink price drop of >50% and Tellor is broken by 0 price: return last good price", async () => {
      ...
      await setTellorPrice(dec(1300, 6))
    ...
    it("C1 chainlinkWorking: Chainlink price drop of >50% and Tellor is broken by stale timestamp: Pricefeed switches to bothOracleSuspect", async () => {
      await setAddresses()
      const statusBefore = await priceFeed.status()
      assert.equal(statusBefore, '0') // status 0: Chainlink working
    ...
    it("C1 chainlinkWorking: Chainlink price drop of >50% and Tellor is broken by stale timestamp: return last good price", async () => {
      await setAddresses()
      priceFeed.setLastGoodPrice(dec(1200, 18)) // establish a "last good price" from the previous price fetch
      ...
      // Make mock stale Tellor
      await th.fastForwardTime(24 * 60 * 60 + 1, web3.currentProvider) // 24 hours
    ...
    it("C1 chainlinkWorking: Chainlink is working and Tellor is working - remain on chainlinkWorking", async () => {
      ...
      await setTellorPrice(dec(103, 18))
    ...
    it("C2 usingTellorChainlinkUntrusted: Tellor breaks by call reverted: switch to bothOraclesSuspect", async () => {
      // deploy broken mock
      mockTellor = await BrokenMockTellor.new(queryData)
      BrokenMockTellor.setAsDeployed(mockTellor)
      tellorCaller = await TellorCaller.new(mockTellor.address, queryId)
      await setAddresses()
    ...
    it("C2 usingTellorChainlinkUntrusted: Tellor breaks by zero timestamp: switch to bothOraclesSuspect", async () => {
      ...
      await mockChainlink.setPrevPrice(dec(999, 8))
      await mockChainlink.setPrice(dec(999, 8))
      await setTellorPrice(dec(999, 6))
    ...
    it("C4 usingTellorChainlinkFrozen: when Chainlink is live and Tellor is live with <5% price difference, switch back to chainlinkWorking", async () => {
      ...
      await setTellorPrice(dec(998, 6))
    ...
    it("C5 usingChainlinkTellorUntrusted: when Chainlink is live and Tellor price >5% - no status change", async () => {
      ...
      await setTellorPrice(dec(123, 6))  // Greater than 5% difference with chainlink
    ...
    it("C5 usingChainlinkTellorUntrusted: when Chainlink is live and Tellor price within <5%, switch to chainlinkWorking", async () => {
      ...
      await setTellorPrice(dec(998, 6))  // within 5% of Chainlink
    ...
    it("C5 usingChainlinkTellorUntrusted: when Chainlink is live, <50% price deviation from previous, Tellor price not within 5%, remain on usingChainlinkTellorUntrusted", async () => {
      ...
      await setTellorPrice(dec(123, 6))  // Tellor not close to current Chainlink
  ...
  context("when Price feed base is being used as price aggregator", () => {
    let priceFeed
    let priceFeedBase
    ...
    const tellorDigits = 18

    const setAddresses = async () => {
      await priceFeed.setAddresses(priceFeedBase.address, tellorCaller.address, { from: owner })
    }
    ...
    describe('Mainnet PriceFeed setup', async accounts => {
      it("setAddresses should fail whe called by nonOwner", async () => {
        await assertRevert(
          priceFeed.setAddresses(priceFeedBase.address, mockTellor.address, { from: alice }),
          "Ownable: caller is not the owner"
        )
      })
      ...
      it("setAddresses should fail after address has already been set", async () => {
        // Owner can successfully set any address
        const txOwner = await priceFeed.setAddresses(priceFeedBase.address, mockTellor.address, { from: owner })
        assert.isTrue(txOwner.receipt.status)

        await assertRevert(
          priceFeed.setAddresses(priceFeedBase.address, mockTellor.address, { from: owner }),
          "PriceFeed: contacts already set"
        )
    ...
    it("C1 Chainlink working: fetchPrice should return the correct price, taking into account the number of decimal digits on the aggregator", async () => {
      ...
      // Oracle price price is 10.00000000
      await mockRedstoneAdapterBase.setPrevPrice(dec(1, 9))
      await mockRedstoneAdapterBase.setPrice(dec(1, 9))
      await priceFeed.fetchPrice()
      let price = await priceFeed.lastGoodPrice()
      // Check Liquity PriceFeed gives 10, with 18 digit precision
      assert.equal(price, dec(10, 18))
      ...
      // Oracle price is 1e9
      await mockRedstoneAdapterBase.setPrevPrice(dec(1, 18))
      await mockRedstoneAdapterBase.setPrice(dec(1, 18))
      await priceFeed.fetchPrice()
      price = await priceFeed.lastGoodPrice()
      // Check Liquity PriceFeed gives 1e9, with 18 digit precision
      assert.isTrue(price.eq(toBN(dec(1, 28))))

      // Oracle price is 0.0001
      await mockRedstoneAdapterBase.setPrevPrice(dec(1, 4))
      await mockRedstoneAdapterBase.setPrice(dec(1, 4))
      await priceFeed.fetchPrice()
      price = await priceFeed.lastGoodPrice()
      // Check Liquity PriceFeed gives 0.0001 with 18 digit precision
      assert.isTrue(price.eq(toBN(dec(1, 14))))

      // Oracle price is 1.23456789
      await mockRedstoneAdapterBase.setPrevPrice(dec(123456789))
      await mockRedstoneAdapterBase.setPrice(dec(123456789))
      await priceFeed.fetchPrice()
      price = await priceFeed.lastGoodPrice()
      // Check Liquity PriceFeed gives the value with 18 digit precision
      assert.equal(price, '1234567890000000000')
    })
    ...
    it("C1 Chainlink breaks, Tellor working: fetchPrice should return the correct Tellor price, taking into account Tellor's 6-digit granularity", async () => {
      ...
      // Chainlink breaks with negative price
      await mockRedstoneAdapterBase.setPrevPrice(dec(1, 8))
      await mockRedstoneAdapterBase.setPrice(dec(1, 4))

      await setTellorPrice(dec(123, 6))
      await mockRedstoneAdapterBase.setUpdateTime(0)
    ...
    it("C1 chainlinkWorking: Chainlink broken by zero latest roundId, Tellor working: switch to usingChainlinkTellorUntrusted", async () => {
      ...
      await mockRedstoneAdapterBase.setPrevPrice(dec(999, 8))
      await mockRedstoneAdapterBase.setPrice(dec(999, 8))
      await priceFeed.setLastGoodPrice(dec(999, 18))

      await setTellorPrice(dec(123, 6))
      await mockRedstoneAdapterBase.setLatestRoundId(0)
    ...
    it("C1 chainlinkWorking: Chainlink broken by future timestamp, Tellor working, switch to usingChainlinkTellorUntrusted", async () => {
      ...
      await setTellorPrice(dec(123, 6))
      await mockRedstoneAdapterBase.setUpdateTime(future)
    ...
    it("C1 chainlinkWorking: Chainlink broken by negative price, Tellor working,  switch to usingChainlinkTellorUntrusted", async () => {
      ...
      await setTellorPrice(dec(123, 6))
      await mockRedstoneAdapterBase.setPrice(dec(1, 8))
    ...
    it("C1 chainlinkWorking: Chainlink broken - latest round call reverted, Tellor working, switch to usingChainlinkTellorUntrusted", async () => {
      ...
      await setTellorPrice(dec(123, 6))
      await mockRedstoneAdapterBase.setLatestRevert()
    ...
    it("C1 chainlinkWorking: previous round call reverted, Tellor working, switch to usingChainlinkTellorUntrusted", async () => {
      ...
      await setTellorPrice(dec(123, 6))
      await mockRedstoneAdapterBase.setPrevRevert()
    ...
    it("C1 chainlinkWorking: Chainlink is out of date by <3hrs: remain chainlinkWorking", async () => {
      ...
      await mockRedstoneAdapterBase.setPrevPrice(dec(1234, 8))
      await mockRedstoneAdapterBase.setPrice(dec(1234, 8))
      await th.fastForwardTime(10740, web3.currentProvider) // fast forward 2hrs 59 minutes
    ...
    it("C1 chainlinkWorking: Chainlink price drop of >50%, switch to usingChainlinkTellorUntrusted", async () => {
      ...
      await setTellorPrice(dec(203, 4))
      await mockRedstoneAdapterBase.setPrevPrice(dec(2, 8))  // price = 2
      await mockRedstoneAdapterBase.setPrice(99999999)  // price drops to 0.99999999: a drop of > 50% from previous
    ...
    it("C1 chainlinkWorking: Chainlink price drop of 50%, remain chainlinkWorking", async () => {
      ...
      await setTellorPrice(dec(203, 4))
      await mockRedstoneAdapterBase.setPrevPrice(dec(2, 8))  // price = 2
      await mockRedstoneAdapterBase.setPrice(dec(1, 8))  // price drops to 1
    ...
    it("C1 chainlinkWorking: Chainlink price drop of <50%, remain chainlinkWorking", async () => {
      ...
      await setTellorPrice(dec(203, 4))
      await mockRedstoneAdapterBase.setPrevPrice(dec(2, 8))  // price = 2
      await mockRedstoneAdapterBase.setPrice(dec(100000001))   // price drops to 1.00000001:  a drop of < 50% from previous
    ...
    it("C1 chainlinkWorking: Chainlink price drop of <50%, return Chainlink price", async () => {
      ...
      await setTellorPrice(dec(203, 4))
      await mockRedstoneAdapterBase.setPrevPrice(dec(2, 8))  // price = 2
      await mockRedstoneAdapterBase.setPrice(100000001)   // price drops to 1.00000001:  a drop of < 50% from previous
    ...
    it("C1 chainlinkWorking: Chainlink price increase of >100%, switch to usingChainlinkTellorUntrusted", async () => {
      ...
      await setTellorPrice(dec(203, 4))
      await mockRedstoneAdapterBase.setPrevPrice(dec(2, 8))  // price = 2
      await mockRedstoneAdapterBase.setPrice(400000001)  // price increases to 4.000000001: an increase of > 100% from previous
    ...
    it("C1 chainlinkWorking: Chainlink price increase of 100%, remain chainlinkWorking", async () => {
      ...
      await setTellorPrice(dec(203, 4))
      await mockRedstoneAdapterBase.setPrevPrice(dec(2, 8))  // price = 2
      await mockRedstoneAdapterBase.setPrice(dec(4, 8))  // price increases to 4: an increase of 100% from previous
    ...
    it("C1 chainlinkWorking: Chainlink price increase of <100%, remain chainlinkWorking", async () => {
      ...
      await setTellorPrice(dec(203, 4))
      await mockRedstoneAdapterBase.setPrevPrice(dec(2, 8))  // price = 2
      await mockRedstoneAdapterBase.setPrice(399999999)  // price increases to 3.99999999: an increase of < 100% from previous
    ...
    it("C1 chainlinkWorking: Chainlink price drop of >50% and Tellor price within 5% of Chainlink: remain chainlinkWorking", async () => {
      ...
      await mockRedstoneAdapterBase.setPrevPrice(dec(1000, 8))  // prev price = 1000
      await mockRedstoneAdapterBase.setPrice(dec(100, 8))  // price drops to 100: a drop of > 50% from previous
      await setTellorPrice(104999999) // Tellor price drops to 104.99: price difference with new Chainlink price is now just under 5%
    ...
    it("C1 chainlinkWorking: Chainlink is working and Tellor is working - remain on chainlinkWorking", async () => {
      ...
      await mockRedstoneAdapterBase.setPrevPrice(dec(101, 8))
      await mockRedstoneAdapterBase.setPrice(dec(102, 8))
    ...
    it("C1 chainlinkWorking: Chainlink is working and Tellor freezes - remain on chainlinkWorking", async () => {
      ...
      await mockRedstoneAdapterBase.setUpdateTime(now) // Chainlink's price is current
    ...
    it("C2 usingTellorChainlinkUntrusted: both Tellor and Chainlink are live and <= 5% price difference - switch to chainlinkWorking", async () => {
      ...
      await setTellorPrice(dec(100, 6)) // price = 100
      await mockRedstoneAdapterBase.setPrice(dec(105, 8)) // price = 105: 5% difference from Chainlink
    ...
    it("C2 usingTellorChainlinkUntrusted: both Tellor and Chainlink are live and > 5% price difference - remain usingChainlinkTellorUntrusted", async () => {
      ...
      await setTellorPrice(dec(100, 6)) // price = 100
      await mockRedstoneAdapterBase.setPrice('10500000001') // price = 105.00000001: > 5% difference from Tellor
    ...
    it("C3 bothOraclesUntrusted: both Tellor and Chainlink are live and <= 5% price difference, switch to chainlinkWorking", async () => {
      ...
      await setTellorPrice(dec(100, 6)) // price = 100
      await mockRedstoneAdapterBase.setPrice(dec(105, 8)) // price = 105: 5% difference from Tellor
    ...
    it("C3 bothOraclesUntrusted: both Tellor and Chainlink are broken, reverts transaction", async () => {
      ...
      await setTellorPrice(0) // Tellor is broken
      await mockRedstoneAdapterBase.setUpdateTime(0) // ChainLink is broken
    ...
    it("C3 bothOraclesUntrusted: Chainlink back online, check ChainLink first, switch to usingChainlinkTellorUntrusted", async () => {
      ...
      await setTellorPrice(0) // Tellor is broken
      await mockRedstoneAdapterBase.setPrevPrice(dec(100, 8))
      await mockRedstoneAdapterBase.setPrice(dec(105, 8)) // price = 105: 5% difference from previous
    ...
    it("C3 bothOraclesUntrusted: Chainlink back online, check Tellor first, switch to usingChainlinkTellorUntrusted", async () => {
      ...
      await mockRedstoneAdapterBase.setPrevPrice(dec(199, 8))
      await mockRedstoneAdapterBase.setPrice(dec(100, 8)) // price = 100: 5% difference from previous
      const now = await th.getLatestBlockTimestamp(web3)
      await mockRedstoneAdapterBase.setUpdateTime(now)
    ...
    it("C3 bothOraclesUntrusted: Tellor back online, check Chainlink first, switch to usingTellorChainlinkUntrusted", async () => {
      ...
      await setTellorPrice(dec(123, 6)) // Tellor is online
      await mockRedstoneAdapterBase.setPrevPrice(dec(201, 8))
      await mockRedstoneAdapterBase.setPrice(dec(100, 8)) // ChainLink: difference from previous more than 50%
    ...
    it("C3 bothOraclesUntrusted: Tellor back online, check Tellor first, switch to usingTellorChainlinkUntrusted", async () => {
      ...
      await mockRedstoneAdapterBase.setPrevPrice(dec(100, 8))
      await mockRedstoneAdapterBase.setPrice(dec(100, 8)) // ChainLink frozen

      await th.fastForwardTime(14400, web3.currentProvider) // Fast forward 4 hours

      // check Chainlink price timestamp is out of date by > 4 hours
      const now = await th.getLatestBlockTimestamp(web3)
      const chainlinkUpdateTime = (await priceFeedBase.latestRoundData())[3]
      assert.isTrue(chainlinkUpdateTime.lt(toBN(now).sub(toBN(14400))))
    ...
    it("C3 bothOraclesUntrusted: both oracles online with different prices, check ChainLink first, switch to usingChainlinkTellorUntrusted", async () => {
      ...
      await mockRedstoneAdapterBase.setPrevPrice(dec(100, 8))
      await mockRedstoneAdapterBase.setPrice(dec(100, 8)) // ChainLink is online
      await setTellorPrice(dec(110, 6)) // Tellor is online, differenece > 5%
    ...
    it("C4 usingTellorChainlinkFrozen: when both Chainlink and Tellor break, switch to bothOraclesSuspect", async () => {
      ...
      // Both Chainlink and Tellor break with 0 price
      await mockRedstoneAdapterBase.setPrice(0)
      await setTellorPrice(0)
    ...
    it("C4 usingTellorChainlinkFrozen: when both Chainlink and Tellor break, return last good price", async () => {
      ...
      // Both Chainlink and Tellor break with 0 price
      await mockRedstoneAdapterBase.setPrice(dec(0))
      await setTellorPrice(dec(0))
    ...
    it("C5 usingChainlinkTellorUntrusted: when Chainlink is live, <50% price deviation from previous, Tellor price not within 5%, remain on usingChainlinkTellorUntrusted", async () => {
      ...
      await mockRedstoneAdapterBase.setPrevPrice(dec(999, 8))
      await mockRedstoneAdapterBase.setPrice(dec(998, 8))
      await setTellorPrice(dec(123, 6))  // Tellor not close to current Chainlink
    ...
    it("C5 usingChainlinkTellorUntrusted: when Chainlink is live, >50% price deviation from previous, Tellor price not within 5%, remain on usingChainlinkTellorUntrusted", async () => {
      ...
      await mockRedstoneAdapterBase.setPrevPrice(dec(200, 8))
      await mockRedstoneAdapterBase.setPrice(dec(99, 8))  // >50% price drop from previous Chainlink price
      await setTellorPrice(dec(123, 6)) // Tellor not close to current Chainlink
    ...
    it("C5 usingChainlinkTellorUntrusted: when Chainlink is live, <50% price deviation from previous, and Tellor is frozen, remain on usingChainlinkTellorUntrusted", async () => {
      ...
      await mockRedstoneAdapterBase.setPrice(dec(998, 8))
      await mockRedstoneAdapterBase.setUpdateTime(now) // Chainlink is current
    ...
    it("C5 usingChainlinkTellorUntrusted: when Chainlink is live, >50% price deviation from previous, Tellor is frozen, remain on usingChainlinkTellorUntrusted", async () => {
      ...
      await mockRedstoneAdapterBase.setPrevPrice(dec(200, 8))
      await mockRedstoneAdapterBase.setPrice(dec(200, 8))
    ...
    it("C5 usingChainlinkTellorUntrusted: when Chainlink breaks too, switch to bothOraclesSuspect", async () => {
      ...
      await mockRedstoneAdapterBase.setPrevPrice(dec(999, 8))
      await mockRedstoneAdapterBase.setPrice(dec(999, 8))
      await mockRedstoneAdapterBase.setUpdateTime(0)  // Chainlink breaks by 0 timestamp
  ...
  context("when Tellor feed has 6 decimals", () => {
    contextTestPriceFeed( 6 )
  })

  context("when Tellor feed has 18 decimals", () => {
    contextTestPriceFeed( 18 )
  })
})
