...
export function updateTotalRedistributed(L_ETH: BigInt, L_THUSDDebt: BigInt): void {
  let global = getGlobal();

  global.rawTotalRedistributedCollateral = L_ETH;
  global.rawTotalRedistributedDebt = L_THUSDDebt;
  global.save();
}
...
export function increaseTotalBorrowingFeesPaid(_THUSDFee: BigInt): void {
  let global = getGlobal();
  global.totalBorrowingFeesPaid = global.totalBorrowingFeesPaid.plus(decimalize(_THUSDFee));
  global.save();
}
