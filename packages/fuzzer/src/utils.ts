...
import { EthersLiquity, ReadableEthersLiquity } from "@threshold-usd/lib-ethers";
import { SubgraphLiquity } from "@threshold-usd/lib-subgraph";
...
export const createRandomTrove = (price: Decimal) => {
  ...
  if (Math.random() < 0.5) {
    ...
    const debt = THUSD_LIQUIDATION_RESERVE.add(truncateLastDigits(maxDebt - benford(maxDebt)));

    return new Trove(collateral, debt);
  } else {
    const debt = THUSD_LIQUIDATION_RESERVE.add(100 * randomValue);
...
export const randomDebtChange = ({ debt }: Trove) =>
  Math.random() < 0.5
    ? { repayTHUSD: debt.mul(1.1 * Math.random()) }
    : { borrowTHUSD: debt.mul(0.5 * Math.random()) };
...
const checks = [
  ...
  new EqualityCheck("tokensInStabilityPool", l => l.getTHUSDInStabilityPool(), decimalsEqual)
];
