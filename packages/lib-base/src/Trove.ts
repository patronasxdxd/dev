...
/** @internal */ export type _THUSDRepayment<T> = { repayTHUSD: T };
...
/** @internal */ export type _NoTHUSDBorrowing = Partial<_THUSDBorrowing<undefined>>;
/** @internal */ export type _NoTHUSDRepayment = Partial<_THUSDRepayment<undefined>>;
...
export type _DebtChange<T> =
  | (_THUSDBorrowing<T> & _NoTHUSDRepayment)
  | (_THUSDRepayment<T> & _NoTHUSDBorrowing);
 ...
 * least one.
 *
 * Defining both `depositCollateral` and `withdrawCollateral`, or both `borrowTHUSD` and `repayTHUSD`
...
const debtChangeFrom = <T>({
  ...
  if (borrowTHUSD !== undefined && repayTHUSD !== undefined) {
    throw new Error(
      "TroveAdjustmentParams: 'borrowTHUSD' and 'repayTHUSD' can't be present at the same time"
    );
  }

  if (borrowTHUSD !== undefined) {
    return { borrowTHUSD };
  }

  if (repayTHUSD !== undefined) {
    return { repayTHUSD };
  }
};
...
export class Trove {
   ...
   */
  get netDebt(): Decimal {
    if (this.debt.lt(THUSD_LIQUIDATION_RESERVE)) {
      throw new Error(`netDebt should not be used when debt < ${THUSD_LIQUIDATION_RESERVE}`);
    }

    return this.debt.sub(THUSD_LIQUIDATION_RESERVE);
  }
  ...
  private _debtChange({ debt }: Trove, borrowingRate: Decimalish): _DebtChange<Decimal> {
    return debt.gt(this.debt)
      ? { borrowTHUSD: unapplyFee(borrowingRate, debt.sub(this.debt)) }
      : { repayTHUSD: this.debt.sub(debt) };
  }
    ...
    if (that.isEmpty) {
      return troveClosure(
        this.netDebt.nonZero
          ? { withdrawCollateral: this.collateral, repayTHUSD: this.netDebt }
