...
const liquidationReserve = Number(THUSD_LIQUIDATION_RESERVE);
const maximumBorrowingRate = Number(MAXIMUM_BORROWING_RATE);

const maxDebt = 10 * Number(THUSD_MINIMUM_DEBT);
        ...
        fc.property(validNonEmptyTrove(), validNonEmptyTrove(), (a, b) => {
          ...
          return (
            ...
            (change.type === "adjustment" &&
              ...
              !change.params.borrowTHUSD?.isZero &&
              !change.params.repayTHUSD?.isZero)
          );
        })
      );
    });
      ...
      fc.assert(
        fc.property(fc.float({ max: maximumBorrowingRate }), borrowingRate => {
          const withMinimumDebt = Trove.recreate(
            new Trove(Decimal.ONE, THUSD_MINIMUM_DEBT),
            borrowingRate
          );

          const ret = Trove.create(withMinimumDebt, borrowingRate).debt.gte(THUSD_MINIMUM_DEBT);
